...
struct FrameFramePrecalc
{
	...
	FrameHessian* host;	// defines row  host代表当前FrameHessian
	FrameHessian* target;	// target代表frames容器（滑动窗口）中的某个FrameHessian对象 defines column

	// precalc values
	Mat33f PRE_RTll;       //host到target的R矩阵 来自frameHession中的PRE_worldToCam
	Mat33f PRE_KRKiTll;     // K*PRE_RTll*Ki
	Mat33f PRE_RKiTll;      // PRE_RTll*Ki
	Mat33f PRE_RTll_0;     //host到target的R矩阵（优化过frameHessians后的矩阵）来自frameHession中的worldToCam_evalPT

	Vec2f PRE_aff_mode;    //是通过函数fromToVecExposure（）计算的host到target的光度和曝光时间的值，用于光度误差计算的a,b;
	float PRE_b0_mode;     //host->aff_g2l_0().b;

	Vec3f PRE_tTll;       //host到target的T矩阵 来自frameHession中的PRE_worldToCam
	Vec3f PRE_KtTll;      // K * PRE_tTll
	Vec3f PRE_tTll_0;     //host到target的T矩阵（优化过frameHessians后的矩阵）来自frameHession中的worldToCam_evalPT

	float distanceLL;     // distanceL是host到target的距离，计算方法为：PRE_tTll.norm();
...
struct FrameHessian
{
	...
	int frameID;		     // incremental ID for keyframes only! （所有关键帧容器allKeyFramesHistory<shell>里的的序号）
	static int instanceCounter;
	int idx;                            //在激活关键帧容器 frameHessians<framehession> （7个）中的序列号；
	...
	Vec10 step;
	Vec10 step_backup;
	Vec10 state_backup; //备份状态值 是state变量的备份
	...
	SE3 PRE_worldToCam;
	SE3 PRE_camToWorld;
    // 大小和frames容器（滑动窗口）大小一样，主要用于存储该帧和frames容器（滑动窗口）里每个元素之间的参数关系（包括自己和自己）
...
struct PointHessian
{
	...
	// static values
	float color[MAX_RES_PER_POINT];			// colors in host frame   对应点周围8个点的像素值
	float weights[MAX_RES_PER_POINT];		// host-weights for respective residuals.  对应点周围8个点的权重
	...
	inline void setIdepthZero(float idepth) {
     ...
     //residuals： 激活点重投影到frameHessions中某一帧所对应的残差对象，在点激活时创建并添加对象到该容器，该容器包含了激活点能进行
     //重投影而不成为外点的所有帧（frameHessions）
	std::vector<PointFrameResidual*> residuals;					// only contains good residuals (not OOB and not OUTLIER). Arbitrary order.
	
    //激活点与frameHessions中的最后一帧和倒数第二帧相对应的PointFrameResidual对象
	...
	void release();
	PointHessian(const ImmaturePoint* const rawPoint, CalibHessian* Hcalib);  //将immature点变为active点的构造函数
