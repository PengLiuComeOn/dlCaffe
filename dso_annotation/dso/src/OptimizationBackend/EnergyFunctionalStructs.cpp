...
{
	std::swap<RawResidualJacobian*>(J, data->J);  // 将EFResidual对象里的雅克比矩阵J与它对应的PointFrameResidual对象里的雅克比矩阵J做交换(用于更新EFResidual的J矩阵)

	Vec2f JI_JI_Jd = J->JIdx2 * J->Jpdd;    // [dx*dx, dx*dy; dx*dy, dy*dy] * 像素坐标对深度求导

	for(int i=0;i<6;i++)  // 6指对pose求导的6项，1次循环取其中其中一项
		JpJdF[i] = J->Jpdxi[0][i]*JI_JI_Jd[0] + J->Jpdxi[1][i] * JI_JI_Jd[1]; //像素坐标对pose求导的矩阵（6*2） * [dx*dx, dx*dy; dx*dy, dy*dy] * 像素坐标对深度求导（2*1）

	JpJdF.segment<2>(6) = J->JabJIdx*J->Jpdd;   //JpJdF(7:8)：  [a的导数*dx,a的导数*dy; b的导数*dx, b的导数*dy] * 像素坐标对深度求导
}
