...
inline int getdir (std::string dir, std::vector<std::string> &files)
{
    ...
    for(int i=0;i<files.size();i++)
    {
        ...
        files[i] = dir + "/" + ss.str() + ".jpg";
//        files[i] = dir + "/" + boost::lexical_cast<std::string>(i) + ".jpg";
    }
...
public:
	ImageFolderReader(std::string path, std::string calibFile, std::string gammaFile, std::string vignetteFile)
	{
		...
		//相机内参矫正和光标定类
		undistort = Undistort::getUndistorterForFile(calibFile, gammaFile, vignetteFile);  //有相机参数则会返回一个Undistort对象；
		...
		// load timestamps if possible. 加载每张图像曝光时间的数据
		loadTimestamps();
	...
	void setGlobalCalibration()
	{
		int w_out, h_out;
		Eigen::Matrix3f K;
		getCalibMono(K, w_out, h_out);       //获得矫正后的相机参数
...
private:
	...
	MinimalImageB* getImageRaw_internal(int id, int unused)
	{
		...
		else
		{
			...
			return IOWrap::readStreamBW_8U(databuffer, readbytes);   //会跳转到未被实现部分，这个函数在哪里被重写了？？
#else
			printf("ERROR: cannot read .zip archive, as compile without ziplib!\n");
			exit(1);
#endif
	...
	inline void loadTimestamps()
	{
		...
		{
			if(exposures[i] == 0)   		// 曝光时间矫正，对已为0的曝光图像用其最近不为零两帧曝光时间的平均值作为它的曝光时间
			{
				// fix!
				float sum=0,num=0;
	...
	std::vector<std::string> files;  //存储所有图片文件名的容器,在构建ImageFolderReader时存进数据；
