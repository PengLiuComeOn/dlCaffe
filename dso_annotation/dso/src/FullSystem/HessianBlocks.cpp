...
void FrameHessian::makeImages(float* color, CalibHessian* HCalib)
{
	...
	{
		dIp[i] = new Eigen::Vector3f[wG[i]*hG[i]];		//dIp用于存储图像像素值，横向和纵向梯度
		absSquaredGrad[i] = new float[wG[i]*hG[i]];   // 每张图的金字塔梯度存储变量：absSquaredGrad[];
	}
	dI = dIp[0];
			...
			for(int y=0;y<hl;y++)
				for(int x=0;x<wl;x++)
				{
					dI_l[x + y*wl][0] = 0.25f * (dI_lm[2*x   + 2*y*wlm1][0] +
												...
												dI_lm[2*x+1 + 2*y*wlm1+wlm1][0]);       //  通过插值将图像缩小为原来的一半，每点的像素值为紧临上一张图四个像素的均值；
				}
		}

		for(int idx=wl;idx < wl*(hl-1);idx++)
		{
			float dx = 0.5f*(dI_l[idx+1][0] - dI_l[idx-1][0]);   	//x梯度，横向间隔一个像素，
			float dy = 0.5f*(dI_l[idx+wl][0] - dI_l[idx-wl][0]);  	//y梯度，纵向间隔一个像素
			...
			dabs_l[idx] = dx*dx+dy*dy;                      //计算x,y方向梯度的距离
...
void FrameFramePrecalc::set(FrameHessian* host, FrameHessian* target, CalibHessian* HCalib )
{
	...
	SE3 leftToLeft_0 = target->get_worldToCam_evalPT() * host->get_worldToCam_evalPT().inverse();  //host到target的pose
