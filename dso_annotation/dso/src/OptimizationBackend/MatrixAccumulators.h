...
{
	H.setZero();
	shiftUp(true);                  //将前面计算的H矩阵值(Data TopRight_Data BotRight_Data)累加给变量Data1m， TopRight_Data1m， BotRight_Data1m
	assert(numIn1==0);
	assert(numIn1k==0);
	...
	for(int r=0;r<10;r++)
		for(int c=r;c<10;c++)
		{
			H(r,c) = H(c,r) = Data1m[idx];   //构造出最终的H矩阵的左上角
			idx++;
		}

	idx=0;
	for(int r=0;r<10;r++)                     //构造出最终的H矩阵的右上角
		for(int c=0; c<3;c++)
		{
	...
	H(10,10) 			= BotRight_Data1m[0];    //构造出最终的H矩阵的右下角
 ...
 */
  inline void update(
		  const float* const x4,  // u对k求导
		  const float* const x6,  // u对pose求导
		  const float* const y4,   // v对k求导
		  const float* const y6,   // v对k求导
		  const float a,          // i对u求导的平方
		  const float b,
		  const float c)          // i对v求导的平方
  {
...
private:
  ...
  float numIn1, numIn1k, numIn1m;  //numIn1记录一帧图像上计算H矩阵累加过程中的点的个数（1个激活点累加使H矩阵累加1次） numIn1m多帧中H矩阵累加点的个数的个数
  ...
  void shiftUp(bool force)
  {
	  if(numIn1 > 1000 || force)
	  {
		  for(int i=0;i<60;i+=4)
			  _mm_store_ps(Data1k+i, _mm_add_ps(_mm_load_ps(Data+i),_mm_load_ps(Data1k+i)));          //对H左上角的矩阵做累加
		  for(int i=0;i<32;i+=4)
			  _mm_store_ps(TopRight_Data1k+i, _mm_add_ps(_mm_load_ps(TopRight_Data+i),_mm_load_ps(TopRight_Data1k+i)));   //对H矩阵右上角矩阵做累加
		  for(int i=0;i<8;i+=4)
			  _mm_store_ps(BotRight_Data1k+i, _mm_add_ps(_mm_load_ps(BotRight_Data+i),_mm_load_ps(BotRight_Data1k+i)));   //对H矩阵右下角矩阵做累加
