...
bool CoarseInitializer::trackFrame(FrameHessian* newFrameHessian, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
    ...
    //ORB-POSE
//    SE3 refToNew_current = newFrameHessian->shell->inputPose;
    ...
    std::cout<<"DSO calculate Pose:"<<std::endl;
    std::cout<<refToNew_current.rotationMatrix()<<std::endl;
    std::cout<<refToNew_current.translation()<<std::endl;
    
    std::cout<<"ORB calculate Pose:"<<std::endl;
    std::cout<<newFrameHessian->shell->inputPose.rotationMatrix()<<std::endl;
    std::cout<<newFrameHessian->shell->inputPose.translation()<<std::endl;
...
void CoarseInitializer::debugPlot(int lvl, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
    ...
    IOWrap::writeImage("/Users/conti-app-ma-033/Desktop/idepth-R.jpg", &iRImg);
		...
		bool plot)
{
	...
	Mat33f RKi = (refToNew.rotationMatrix() * Ki[lvl]).cast<float>();    //获得SE3转换的旋转矩阵
	Vec3f t = refToNew.translation().cast<float>();      //获得SE3转换的平移矩阵
	...
	Accumulator11 E;
	acc9.initialize();
	E.initialize();        //初始化acc9和E的成员变量; 将所有成员变量值置为0
	...
	for(int i=0;i<npts;i++)
	{
		...
		{
			int dx = patternP[idx][0];    //{0,-2},{-1,-1},{1,-1},{-2,0},{0,0},{2,0},{-1,1},{0,2}
			int dy = patternP[idx][1];


			Vec3f pt = RKi * Vec3f(point->u+dx, point->v+dy, 1) + t*point->idepth_new;  //p=K(RP+t)
			float u = pt[0] / pt[2];
			float v = pt[1] / pt[2];
			float Ku = fxl * u + cxl;    //投影到第二帧图像的x坐标
			float Kv = fyl * v + cyl;    //投影到第二帧图像的y坐标
			float new_idepth = point->idepth_new/pt[2];    //到第二帧图像相机坐标系下的逆深度
			...
			float dxdd = (t[0]-t[2]*u)/pt[2];   //target图像的u对逆深度求导 (中间的一个过程，fx/z补充在后面)
			float dydd = (t[1]-t[2]*v)/pt[2];   //target图像的v对逆深度求导
...
void CoarseInitializer::propagateDown(int srcLvl)
{
	...
	int nptst= numPoints[srcLvl-1];
	Pnt* ptss = points[srcLvl];//上层
	Pnt* ptst = points[srcLvl-1];//下层
...
void CoarseInitializer::setFirst(CalibHessian* HCalib, FrameHessian* newFrameHessian)
{
	...
	makeK(HCalib);    //计算图像各层金字塔的K矩阵和K矩阵的逆；
	firstFrame = new FrameHessian;

	PixelSelector sel(w[0],h[0]);     //定义一个点选择器
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		int npts;
		if(lvl == 0)
			npts = sel.makeMaps(firstFrame, statusMap,densities[lvl]*w[0]*h[0],1,false,2);   //获取候选点
		else 
		    //根据每层的高斯梯度为每个像素设置状态位，返回状态位为1的点的个数；
		...
		for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		{
			...
			if((lvl!=0 && statusMapB[x+y*wl]) || (lvl==0 && statusMap[x+y*wl] != 0))
			{
				...
				Eigen::Vector3f* cpt = firstFrame->dIp[lvl] + x + y*w[lvl];//lvl层金字塔上x,y位置的像素值，横向纵向梯度；
				float sumGrad2=0;
				for(int idx=0;idx<patternNum;idx++)
				{
