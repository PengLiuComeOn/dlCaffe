		...
		ImmaturePointTemporaryResidual* residuals)
{
	...
	float lastEnergy = 0;
	float lastHdd=0;         //linearizeResidual()-->lastHdd += (hw*d_idepth)*d_idepth;  GN优化中的H矩阵
	float lastbd=0;          //linearizeResidual()-->lastbdbd += (hw*residual)*d_idepth; GN优化中的b矩阵
	float currentIdepth=(point->idepth_max+point->idepth_min)*0.5f;

	for(int i=0;i<nres;i++)
	{
		//将immature点重投影到frameHessians上的其他帧上，返回对应帧的误差
        lastEnergy += point->linearizeResidual(&Hcalib, 1000, residuals+i,lastHdd, lastbd, currentIdepth);
		residuals[i].state_state = residuals[i].state_NewState;    //更新immature点重投影到frameHessians后的状态
		residuals[i].state_energy = residuals[i].state_NewEnergy;  //更新immature点重投影到frameHessians后的误差
	}
	...
	float lambda = 0.1;
    //GN优化方法，优化计算idepth  由于点只有一个，变量只有一个所以H和b矩阵只有一个元素
	for(int iteration=0;iteration<setting_GNItsOnPointActivation;iteration++)
	{
		...
		if(fabsf(step) < 0.0001*currentIdepth)    //GN优化方法里迭代结束的判断条件
			break;
	}
	...
	for(int i=0;i<nres;i++)
		if(residuals[i].state_state == ResState::IN)
		{
            ...
            //对应PointHessian点，插入PointFrameResidual到Residuals中
			p->residuals.push_back(r);
            
            //如果目标帧（被投影帧）为frameHessians的最后一帧（该帧为即将插入的关键帧），或则为倒数第二帧；
			if(r->target == frameHessians.back())
            {
                //对应PointHessian点，插入PointFrameResidual到lastResiduals中并设置状态
