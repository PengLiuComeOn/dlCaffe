...
namespace L3DPP
{
    ...
    public:
        ...
        void addLineFile(std::string filePath);
        ...
        void saveResultAsSTL(const std::string& output_folder, std::map<unsigned int,Eigen::Vector3d> wps_coords);
    ...
    private:
        ...
        boost::mutex display_img_mutex_;

        // view data
        unsigned int num_lines_total_;
        boost::mutex view_mutex_;
        boost::mutex epipolar_mutex_;

        boost::mutex view_reserve_mutex_;
        boost::mutex scoring3D_mutex_;
        std::vector<unsigned int> view_order_; // save the image id , the view object created by the image
        std::map<unsigned int,L3DPP::View*> views_; // first: image id, second: the view object that correspond the image
        std::set<unsigned int> views_reserved_;
        std::vector<float> views_avg_depths_;  // the init val is the median_depth
        ...
        std::map<unsigned int,std::list<unsigned int> > worldpoints2views_; // first: 3d Point id , second: the image Ids that can see the 3d point
        std::map<unsigned int,std::list<unsigned int> > views2worldpoints_; // first : image id, second: 3d Point id that can be see by the image
        std::map<unsigned int,std::list<unsigned int> > fixed_visual_neighbors_; //first: image id, second:the image ids that can match with the image
        std::map<unsigned int,unsigned int> num_worldpoints_;   // first: image id, second:the number of 3d Point
        std::map<unsigned int,std::set<unsigned int> > visual_neighbors_;  //first: image id, second: the neighbor image ids;
        ...
        std::map<unsigned int,std::map<unsigned int,Eigen::Matrix3d> > fundamentals_; // first: src imageId, second.first target image Id, save F that from src image to target image；
        std::map<unsigned int,std::vector<std::list<L3DPP::Match> > > matches_;  // first: image id , second: the matched object info that correspond with each detected line;
        std::map<unsigned int,unsigned int> num_matches_;
        std::map<unsigned int,bool> processed_;

        std::vector<std::pair<int,std::vector<std::pair<cv::Point, cv::Point> > > > lineInfo_;
