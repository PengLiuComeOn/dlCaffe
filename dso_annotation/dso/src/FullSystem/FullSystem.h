...
public:
	...
	bool initFailed;
	bool initialized;
	bool linearizeOperation;// 是否线性化的状态变量

	// 加载初始初始位姿
    void loadPosePrecalc(const std::string& strPose);
    void sortPosePrecalc();
    
	void setGammaFunction(float* BInv);
	void setOriginalCalib(VecXf originalCalib, int originalW, int originalH);
    
    std::vector<SE3> vPose;   //SE3容器存储位姿
    int firstFrameId;
...
private:
	...
	long int statistics_numActivatedPoints;    //统计总的激活的点的个数；
