...
{
    //------------------------------------------------------------------------------
    Line3D::Line3D(const std::string& output_folder,
                   const bool load_segments,
...

    {
        ...
        std::cout << "start to load line data... " << std::endl;
        std::string linePath = "./sparse/lineInfo.txt";
        addLineFile(linePath);
        std::cout << "load line data success... " << std::endl;

        display_text_mutex_.unlock();

        int lineInfoSize = static_cast<int>(lineInfo_.size());

        for (int lineInfoId = 0; lineInfoId < lineInfoSize; lineInfoId++)
        {
            std::cout << lineInfo_[lineInfoId].first << std::endl;

            for(size_t i=0; i < lineInfo_[lineInfoId].second.size(); ++i)
            {
                float4 coordsf4;
                std::pair<cv::Point, cv::Point> &pntPair = lineInfo_[lineInfoId].second[i];
                coordsf4.x = pntPair.first.x;
                coordsf4.y = pntPair.first.y;
                coordsf4.z = pntPair.second.x;
                coordsf4.w = pntPair.second.y;
            }
    ...
    //------------------------------------------------------------------------------
    void Line3D::addImage(const unsigned int camID, cv::Mat& image,     // rgbImg
                          ...
                          const std::vector<cv::Vec4f>& line_segments)
    {
        ...
        if(line_segments.size() == 0)
        {
            if(0)
            {
                ...
                float paremeterS = 0.60;
                float paremeterE = 0.99;
                cv::Mat cutImg = image.rowRange(image.rows * paremeterS, image.rows * paremeterE).colRange(0, image.cols).clone();
//                 cv::Mat grayImg, edgeImg;
//                 cv::cvtColor(cutImg,grayImg,CV_BGR2GRAY);
//                 cv::Canny(grayImg,edgeImg,50,150,3);


                lines = detectLineSegments(camID,cutImg);
                for(size_t m=0; m < lines->width(); ++m)
                {
                        lines->dataCPU(m,0)[0].y += image.rows*paremeterS;
                        lines->dataCPU(m,0)[0].w += image.rows*paremeterS;
                }
            }
            else
            {
                ...
                for (int lineInfoId = 0; lineInfoId < lineInfoSize; lineInfoId++)
                {
                    if (camID == lineInfo_[lineInfoId].first)
                    {
                        lines = new L3DPP::DataArray<float4>(lineInfo_[lineInfoId].second.size(),1);
                        for(size_t i = 0; i < lineInfo_[lineInfoId].second.size(); ++i)
                        {
                            float4 coordsf4;
                            std::pair<cv::Point, cv::Point> &pntPair = lineInfo_[lineInfoId].second[i];
        ...
        if(0)
        {
            display_img_mutex_.lock();
            cv::Mat imgShow = image.clone();
            for(size_t m = 0; m < lines->width(); ++m)
            {
                cv::Point pts1, pts2;
                pts1.x = lines->dataCPU(m,0)[0].x;
                pts1.y = lines->dataCPU(m,0)[0].y;
                pts2.x = lines->dataCPU(m,0)[0].z;
                pts2.y = lines->dataCPU(m,0)[0].w;
                cv::line(imgShow, pts1, pts2, cv::Scalar(0, 255, 255), 2);
            }

            cv::imshow("imgShow", imgShow);
            cv::waitKey(0);
            display_img_mutex_.unlock();
        }
        ...
        // create view
        L3DPP::View* v = new L3DPP::View(camID,lines,K,R,t,image.cols,image.rows,median_depth, image);
        view_mutex_.lock();
        ...
        if(neighbors_by_worldpoints_)   // neighbors_by_worldpoints_ = true;
        {
    ...
    void Line3D::addLineFile(std::string filePath)
    {
        ...
        if (fileIn.is_open())
        {
            ...
            while(std::getline(fileIn, strLine))
            {
                ...
                cv::Point fsPnt;
                cv::Point scdPnt;
                std::vector<std::pair<cv::Point, cv::Point> > linePair;
                ...
                while(iss >> val)
                {
                    ...
                    else if (3 == count)
                    {
                        ...
                        std::pair<cv::Point, cv::Point> pairPnts(fsPnt, scdPnt);

                        linePair.push_back(pairPnts);
                    }
                }

                std::pair<int,std::vector<std::pair<cv::Point, cv::Point> > > imgPair(imgId, linePair);
                lineInfo_.push_back(imgPair);
            }
        ...
        {
            std::cout << "the lineInfo file can not be open!" << std::endl;
        }
    }
...
//        else
    ...
    //------------------------------------------------------------------------------
    //sigma_position: 2.5 sigma_angle: 10.0, num_neighbors: 10 (the minest is 2), epipolar_overlap: 0.25 (the max is 0.99) kNN: 10  const_regularization_depth: -1
                             ...
                             const int kNN, const float const_regularization_depth)
    {
        ...
        // compute spatial regularizer  fixed3Dregularizer_ = false;
        ...
        if(const_regularization_depth_ < 0.0f && fixed3Dregularizer_ && views_avg_depths_.size() > 0) //false
        {
        ...
        for(size_t i=0; i<view_order_.size(); ++i)
        {
            ...
            if(!fixed3Dregularizer_)  //fixed3Dregularizer_: false
                views_[camID]->computeSpatialRegularizer(sigma_p_);   //change k_ for each frame
            else
        ...
        for(size_t i=0; i<view_order_.size(); ++i)
        {
            ...
            if(fixed_visual_neighbors_.find(camID) != fixed_visual_neighbors_.end())  //fixed_visual_neighbors_ is empty;
            {
    ...
    void Line3D::translate()
    {
        ...
        translation_ = Eigen::Vector3d(0,0,0);  //translate Matrix: the value get from optical center mid value
        ...
        for(size_t i=0; i<3; ++i)
        {
            ...
            for(; it!=views_.end(); ++it)
            {
                double val = (it->second->C())(i); // first get x , second get y, third get z
                if(fabs(val) > L3D_EPS)
    ...
    void Line3D::performTranslation(const Eigen::Vector3d t)
    {
        ...
        for(size_t i=0; i<view_order_.size(); ++i)
        {
            views_[view_order_[i]]->translate(t);  //calcualte new R,t   translate(t)--> C_ += t;  t_ = -R_ * C_;  //(R is same, t is -R*t, a inverse translation from optical to T)
        }
        ...
        #pragma omp parallel for
#endif //L3DPP_OPENMP
        for(size_t i=0; i<lines3D_.size(); ++i)  // lines3D_.size() = 0 , skip the circlation
        {
    ...
    {
        if(visual_neighbors_.find(camID) != visual_neighbors_.end()) //visual_neighbors_ was set empty set object;
        {
            // reset
            visual_neighbors_[camID].clear();
            std::map<unsigned int,unsigned int> commonWPs;

            std::list<unsigned int>::const_iterator wp_it = views2worldpoints_[camID].begin();  // all the 3d points that can be seen by the camID
            for(; wp_it!=views2worldpoints_[camID].end(); ++wp_it)
            {
                // iterate over worldpoints
                unsigned int wpID = *wp_it;

                std::list<unsigned int>::const_iterator view_it = worldpoints2views_[wpID].begin(); // all the frameId that can see the 3d point
                for(; view_it!=worldpoints2views_[wpID].end(); ++view_it)
                {
                    // all views are potential neighbors
                    unsigned int vID = *view_it;
                    if(vID != camID)  // skip itself
                    {
                        if(commonWPs.find(vID) == commonWPs.end()) // the first statistic
                        {
                            commonWPs[vID] = 1;
                        }
                        else
                        {
                            ++commonWPs[vID];          // statistic the all number of 3d points can be seen by camID and vID at the same time
                        }
                    }
                }
            }
            ...
            for(; c_it!=commonWPs.end(); ++c_it)
            {
                ...
                VisualNeighbor vn;
                vn.camID_ = vID;
                vn.score_ = 2.0f*float(num_common_wps)/float(num_worldpoints_[camID]+num_worldpoints_[vID]); // calculate a score between neighbor image: the score is : 2 multi the common points and divide the all 3d point in camId and vID;
                vn.axisAngle_ = v->opticalAxesAngle(views_[vID]); // 计算camId和vID的相机在世界坐标系的相机朝向(光心射线)的夹角， 该夹角可以衡量相机共视区域的大小
                vn.distance_score_ = v->distanceVisualNeighborScore(views_[vID]);  // views_[vID]的光心在views_[camID]的相机坐标系下的的x坐标值与y坐标值之和， 该也值可以衡量相机共视区域的大小
            ...
            // sort by score
            neighbors.sort(L3DPP::sortVisualNeighborsByScore); //根据score值从大到小排序

            // reduce to best neighbors
            if(neighbors.size() > num_neighbors_)  //num_neighbors_ = 10
            {
                // copy neighbors
                ...
                neighbors.resize(num_bigger_t);  //删除大于num_bigger_t后面的元素

                // resort based on projective_score and world_point_score
                neighbors.sort(L3DPP::sortVisualNeighborsByDistScore); ////根据distance_score_值从大到小排序

                if(neighbors.size() > num_neighbors_/2)
                    neighbors.resize(num_neighbors_/2);  //删除大于num_neighbors_/2后面的元素

                // combine
                neighbors.splice(neighbors.end(),neighbors_tmp);  // 将neighbors_tmp的所有元素插入到neighbors之后，并清空neighbors_tmp
            }

            // highscore neighbors -> store in visual neighbor map
            float min_baseline = v->getSpecificSpatialReg(0.5f)*v->median_depth();  // v->median_depth() = 0
            min_baseline = 0.1f;
            std::list<L3DPP::VisualNeighbor>::const_iterator nit = neighbors.begin();

            // 过滤掉光心距离过近的临近帧，使任意两帧都间隔一定距离
            while(nit!=neighbors.end() && used_neighbors.size() < num_neighbors_) //num_neighbors_ = 10
            {
                L3DPP::VisualNeighbor vn = *nit;
                L3DPP::View* v2 = views_[vn.camID_];

                // check baseline
                if(used_neighbors.find(vn.camID_) == used_neighbors.end()) //&& v->baseLine(v2) > min_baseline) // min_baseline = 0.1
                {
//                    std::set<unsigned int>::const_iterator u_it = used_neighbors.begin();
//                    bool valid = true;
//                    for(; u_it!=used_neighbors.end() && valid; ++u_it)
//                    {
                    // if(!(v->baseLine(views_[*u_it]) > min_baseline))  // 计算相机光心的模长，基线长度
//                            valid = false;
//                    }
//
//                    if(valid)
    ...
    {
        std::map<unsigned int,std::set<unsigned int> >::const_iterator it = visual_neighbors_.begin();  //在findVisualNeighborsFromWPs函数中已为visual_neighbors_赋值；
        for(; it != visual_neighbors_.end(); ++it)
        {
            ...
            std::cout << "[" << std::setfill('0') << std::setw(L3D_DISP_CAMS) << it->first << "] --> "; //如果it->first = 12 则输出：[0012]-->
            ...
            for(; n_it != it->second.end(); ++n_it)
            {
                if(matched_[it->first].find(*n_it) == matched_[it->first].end())
                {
                    ...
                    // compute fundamental matrix F is from src Image Id to neighbor image Id;
...
//                     else
//                     std::cout << "matching CPU!" << std::endl;
    ...
    Eigen::Matrix3d Line3D::getFundamentalMatrix(L3DPP::View* src, L3DPP::View* tgt)
    {
        ...
        // 使用下标访问map的元素时，如果键不存在，则为创建该键元素， 值为空， fundamentals_[src->id()].find(tgt->id())中不存在src->id()时会自动创建该元素。
        ...
        Eigen::Matrix3d R = R2 * R1.transpose(); // 计算 R1 (src) --> R2 (tgt) 的旋转矩阵
        Eigen::Vector3d t = t2 - R * t1;         // 计算 t1 (src) --> t2 (tgt) 的平移向量； 计算方式： T1 = [R1, t1; 0, 1] --> T1.inv() = [R1.t(), -R1.t() * t1; 0, 1 ]; T2 = [R2, t2; 0, 1];
                                                 // T1 (src) --> T2 (tgt)  --> T2 * T1.inv() = [R2 * R1.t(), t2 - R2 * R1.t() * t1; 0, 1]

        Eigen::Matrix3d T(3,3);  // 构建平移向量的反对称矩阵；
        T(0,0) = 0.0;    T(0,1) = -t.z(); T(0,2) = t.y();
        T(1,0) = t.z();  T(1,1) = 0.0;    T(1,2) = -t.x();
        T(2,0) = -t.y(); T(2,1) = t.x();  T(2,2) = 0.0;

        // 计算本质矩阵： E = [Tx] * R; [Tx]为平移向量的反对称矩阵
        // 计算基础矩阵： F = K.t().inv() * E * K.t();  基础矩阵F的物理意义： m2.t() * F * m1 = 0 --> 已知图像上一点m1'和F矩阵可以求的它在另外一幅图像中的坐标m2;
        // 注： R， t 是从1到2的变换矩阵， m1为1中的图像的2d点， m2为2中的图像的2d点；
        // 基础矩阵的作用： F*m1 为1中图像的点m1在2中图像中的对极线； m2*F：为2中的图像点m2, 在图像1中的对极线；
                             ...
                             const Eigen::Matrix3d& F)
    {
        ...
        cv::Mat srcIMG, tgtIMG;
        srcIMG = v_src ->imgRgb_;
        tgtIMG = v_tgt ->imgRgb_;
        unsigned int num_matches = 0;

        char imgName1[40], imgName2[40];
        sprintf(imgName1, "srcID: %d, lineNum: %d", static_cast<int>(src), lines_src -> width());
        sprintf(imgName2, "tgtID: %d, lineNum: %d", static_cast<int>(tgt), lines_tgt -> width());
        ...
        for(size_t r = 0; r < lines_src -> width(); ++r)
        {
            ...
            // epipolar lines
            Eigen::Vector3d epi_p1 = F*p1;  //计算p1点在target图像中的对极线参数
            Eigen::Vector3d epi_p2 = F*p2;  //计算p2点在target图像中的对极线参数
            ...
            for(size_t c = 0; c < lines_tgt -> width(); ++c)
            {
                ...
                Eigen::Vector3d l2 = q1.cross(q2);  // 平面上两个点的齐次坐标做外积得到的是两个点的连线参数；

                // intersect
                Eigen::Vector3d p1_proj = l2.cross(epi_p1); //平面上两条线做外积得到的是这两条线的交点；
                Eigen::Vector3d p2_proj = l2.cross(epi_p2);

                if(fabs(p1_proj.z()) > L3D_EPS && fabs(p2_proj.z()) > L3D_EPS)
                {
                    // normalize
                    p1_proj /= p1_proj.z();  // 归一化后p1_proj.x, p1_proj.y就具体对应到图像中的像素位置
                    p2_proj /= p2_proj.z();

                    // check overlap
                    std::vector<Eigen::Vector3d> collinear_points(4);  //存储四个点: 2.与对极线的交点 1.检测到的线的端点
                    ...
                    // check the angle and distance for epipolar lines
                    float dot_l2ep1 = fabs(l2.x() * epi_p1.x() + l2.y() * epi_p1.y());
                    float dot_l2ep2 = fabs(l2.x() * epi_p2.x() + l2.y() * epi_p2.y());
                    float len_l2 = sqrt(l2.x() * l2.x() + l2.y() * l2.y());
                    float len_ep1 = sqrt(epi_p1.x() * epi_p1.x() + epi_p1.y() * epi_p1.y());
                    float len_ep2 = sqrt(epi_p2.x() * epi_p2.x() + epi_p2.y() * epi_p2.y());
                    float angle1 = acos(dot_l2ep1/len_l2/len_ep1)/M_PI*180.0f;
                    float angle2 = acos(dot_l2ep1/len_l2/len_ep1)/M_PI*180.0f;
                    float disq1ep1 = fabs( epi_p1.x()*q1.x() + epi_p1.y()*q1.y() + epi_p1.z()) / len_ep1;
                    float disq2ep1 = fabs( epi_p1.x()*q2.x() + epi_p1.y()*q2.y() + epi_p1.z()) / len_ep1;
                    float avgDisEq1 = 0.5*(disq1ep1 + disq2ep1);
                    float disq1ep2 = fabs( epi_p2.x()*q1.x() + epi_p2.y()*q1.y() + epi_p2.z()) / len_ep2;
                    float disq2ep2 = fabs( epi_p2.x()*q2.x() + epi_p2.y()*q2.y() + epi_p2.z()) / len_ep2;
                    float avgDisEq2 = 0.5*(disq1ep2 + disq2ep2);

                    if(score > epipolar_overlap_ || (angle1 < 5.0f && angle2 < 5.0f && avgDisEq1 < 25.0f && avgDisEq2 < 25.0f))
                    {
                        if(0)  // for debug.
                        {
                            // original lines in img1
                            cv::Mat tempSrc = srcIMG.clone();
                            cv::Mat tempTgt = tgtIMG.clone();
                            cv::Point pts11, pts12;
                            pts11.x = p1[0];
                            pts11.y = p1[1];
                            pts12.x = p2[0];
                            pts12.y = p2[1];
                            cv::line(tempSrc, pts11, pts12, cv::Scalar(0, 255, 0), 2);

                            // img1 to img2: epipolar
                            cv::Point epts11, epts12, epts21, epts22;
                            epts11.x = 0;
                            epts11.y = -(epi_p1[0] * epts11.x + epi_p1[2])/epi_p1[1];
                            epts12.x = tempSrc.cols;
                            epts12.y = -(epi_p1[0] * epts12.x + epi_p1[2])/epi_p1[1];

                            epts21.x = 0;
                            epts21.y = -(epi_p2[0] * epts21.x + epi_p2[2])/epi_p2[1];
                            epts22.x = tempSrc.cols;
                            epts22.y = -(epi_p2[0] * epts22.x + epi_p2[2])/epi_p2[1];
                            cv::line(tempTgt, epts11, epts12, cv::Scalar(0, 255, 0), 2);
                            cv::line(tempTgt, epts21, epts22, cv::Scalar(0, 255, 0), 2);

                            // img2 detect lines and intersect
                            cv::Point pts21, pts22, inpts1, inpts2;
                            pts21.x = q1[0];
                            pts21.y = q1[1];
                            pts22.x = q2[0];
                            pts22.y = q2[1];
                            cv::line(tempTgt, pts21, pts22, cv::Scalar(0, 0, 255), 2);
                            inpts1.x = p1_proj[0];
                            inpts1.y = p1_proj[1];
                            inpts2.x = p2_proj[0];
                            inpts2.y = p2_proj[1];
                            cv::circle(tempTgt, inpts1, 3, cv::Scalar(255, 255, 0), 2);
                            cv::circle(tempTgt, inpts2, 3, cv::Scalar(255, 255, 0), 2);

                            std::cout << "score: " << score << std::endl;
                            std::cout << "angle1: " << angle1 << std::endl;
                            std::cout << "angle2: " << angle2 << std::endl;
                            std::cout << "avgDisEq2: " << avgDisEq2 << std::endl;

                            cv::namedWindow(imgName1, CV_WINDOW_NORMAL);
                            cv::namedWindow(imgName2, CV_WINDOW_NORMAL);
                            cv::imshow(imgName1, tempSrc);
                            cv::imshow(imgName2, tempTgt);
                            cv::waitKey(0);

                            cv::destroyWindow(imgName1);
                            cv::destroyWindow(imgName2);
                        }
    ...
    void Line3D::scoringCPU(const unsigned int src, float& valid_f)
    {
        ...
        // iterative scoring
        scoring3D_mutex_.lock();
        for(size_t i = 0; i < matches_[src].size(); ++i)  // the src-th view.
        {
            bool valid_match_exists = false;

            std::list<L3DPP::Match>::iterator it = matches_[src][i].begin();
            for(; it != matches_[src][i].end(); ++it)  // matched tgt view
            {
                L3DPP::Match M = *it;
                float score3D = 0.0f;
                ...
                for(; it2 != matches_[src][i].end(); ++it2)  // matched lines..
                {
                    L3DPP::Match M2 = *it2;
...
//                     std::cout << "score3D:" << score3D << std::endl;
//                     getchar();
                }
    ...
    void Line3D::filterMatches(const unsigned int src)
    {
        ...
        for(size_t i = 0; i < matches_[src].size(); ++i)
        {
            ...
            if(best_match.score3D_ > 0.01) //L3D_DEF_MIN_BEST_SCORE_3D
            {
    ...
    //------------------------------------------------------------------------------
    void Line3D::saveResultAsSTL(const std::string& output_folder, std::map<unsigned int,Eigen::Vector3d> wps_coords)
    {
        view_mutex_.lock();
        view_reserve_mutex_.lock();
...
//        std::map<unsigned int,Eigen::Vector3d>::const_iterator it = wps_coords.begin();
//        for (; it != wps_coords.end(); ++it)
//        {
...
//        }
                                     ...
                                     const std::vector<L3DPP::Segment3D>& lines3D)
    {
        ...
        std::string filename2 = output_folder+"/"+createOutputFilename()+"__"+suffix+".txt";
        std::ofstream file2;
