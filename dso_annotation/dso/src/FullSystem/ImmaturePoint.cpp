...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
    ...
    //如果最大深度无穷大则用maxPixSearch指定的范围内搜
	else
	{
		dist = maxPixSearch;
    ...
    // gradH是根据对应像素临近8个像素的像素梯度和（x,y）计算得到的2*2矩阵
    ...
    //通过重投影公式，对于坐标进行分解（u+dx,v+dx,1）分解为（u,v,1）和（dx,dy,0）带入计算得到（u,v）相邻点的重投影坐标
	for(int idx=0;idx<patternNum;idx++)
	...
	//精细迭代优化匹配位置 find best score outside a +-2px radius.
	...
	if(newQuality < quality || numSteps > 10) quality = newQuality;  //quality作为激活点的一个判断条件
	...
	for(int it=0;it<setting_trace_GNIterations;it++)
	{
		...
		else
		{
			...
			float step = -gnstepsize*b/H;     //计算步长，以便优化；
...
//			 && absGrad2*areaGradientSlackFactor < host->frameGradTH*0.50f))
	...
	// ============== set new inverse depth interval ===================
	if(dx*dx>dy*dy)
	{
	...
	lastTracePixelInterval=2*errorInPixel;    //作为激活点的一个判断条件
		...
		float idepth)
{
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		float drescale, u, v, new_idepth;  //drescale投影到3D空间对应的Z坐标
		float Ku, Kv;   //重投影坐标
		Vec3f KliP;

		if(!projectPoint(this->u,this->v, idepth, dx, dy,HCalib,
				PRE_RTll,PRE_tTll, drescale, u, v, Ku, Kv, KliP, new_idepth))   //计算对应immature点周围的8个像素的重投影坐标
			{tmpRes->state_NewState = ResState::OOB; return tmpRes->state_energy;}   //重投影坐标超出图像范围，则返回


		Vec3f hitColor = (getInterpolatedElement33(dIl, Ku, Kv, wG[0]));  //插值法计算对应位置像素值和梯度

		if(!std::isfinite((float)hitColor[0])) {tmpRes->state_NewState = ResState::OOB; return tmpRes->state_energy;}
		float residual = hitColor[0] - (affLL[0] * color[idx] + affLL[1]); // 重投影误差
		...
		Hdd += (hw*d_idepth)*d_idepth; //求H矩阵(累加周围8个像素的导数)，由于点只有一个，变量只有一个所以H和b矩阵只有一个元素
		bd += (hw*residual)*d_idepth;  //求b矩阵(累加周围8个像素的导数)
	}
