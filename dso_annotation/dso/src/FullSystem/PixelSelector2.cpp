...
{
	gradHistFrame = fh;
	float * mapmax0 = fh->absSquaredGrad[0];  //前面高斯金字塔已经计算出absSquaredGrad（梯度）的值了
	...
	//将图像分为多个小区域，每个小区域的大小为32*32
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			float* map0 = mapmax0+32*x+32*y*w;                   //map0指向每个小块的起点（每个小块大小为32*32）
			...
			for(int j=0;j<32;j++) for(int i=0;i<32;i++)
			{
			   ...
			   //对直方图进行统计,对每个小区域计算出一个统计结果：统计的分布值，取出中位数，加上全局的常量7，作为当前块的阈值
            ths[x+y*w32] = computeHistQuantil(hist0,setting_minGradHistCut) + setting_minGradHistAdd;
		}
	 
	 
	//对每个小区域计算它相邻的8个区域和它本身的ths均值， 最终对每个区域计算一个对应的平滑值；
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			float sum=0,num=0;
...
//	else
	{
		...
		if( recursionsLeft>0 && quotia > 1.25 && currentPotential>1)     //选点个数不够，降低阈值再次选点；
		{
		...
		float* map_out, int pot, float thFactor)
{
 	...
 	Eigen::Vector3f const * const map0 = fh->dI;		//dI存储有第0层的图像像素值，横向和纵向梯度
...
/* 	
*   将每帧图像分为4d*4d，4d*4d小图像又分为2d*2d,2d*2d小图像又分为d*d的图像(d=3)，依次遍历每个d*d的图像；
* 	选择策略：1. 在d*d区域从第一层金字塔上选择第一类候选点，如果在某一个d*d区域上有第一类候选点，则在后面几个
* 	d*d区域不在第二，三层金子塔选择第二，三类侯选点（这几个d*d区域应同时包含在一个2d*2d区域）；同样方式在2d*2d
* 	区域从第二层金字塔上选择第二类候选点；在4d*4d区域从第三层金字塔上选择第三类候选点。
*   一个d*d区域最多选择一个一类候选点，一个2d*2d区域最多选择一个第二类候选点，一个4d*4d区域最多选择1个三类候选点；
*/
	int n3=0, n2=0, n4=0;
		...
		for(int y3=0;y3<my3;y3+=(2*pot)) for(int x3=0;x3<mx3;x3+=(2*pot))
		{
			...
			for(int y2=0;y2<my2;y2+=pot) for(int x2=0;x2<mx2;x2+=pot)
			{
				...
				for(int y1=0;y1<my1;y1+=1) for(int x1=0;x1<mx1;x1+=1)
				{
					assert(x1+x234 < w);
					assert(y1+y234 < h);
					int idx = x1+x234 + w*(y1+y234);    //idx：对应像素位置
					int xf = x1+x234;
					int yf = y1+y234;
					...
					float pixelTH0 = thsSmoothed[(xf>>5) + (yf>>5) * thsStep];      //查询对应像素点所在区域的平滑阈值；
					float pixelTH1 = pixelTH0*dw1;
					float pixelTH2 = pixelTH1*dw2;


					float ag0 = mapmax0[idx];            //ag0：该像素对应的第0层高斯梯度值
					if(ag0 > pixelTH0*thFactor)
					{
						Vec2f ag0d = map0[idx].tail<2>();    //ag0d：对应像素的横向和纵向梯度；
						float dirNorm = fabsf((float)(ag0d.dot(dir2)));//求梯度与dir2的内积； 内积是代表余弦；
					...
					float ag1 = mapmax1[(int)(xf*0.5f+0.25f) + (int)(yf*0.5f+0.25f)*w1];   //ag1：该像素对应的第1层高斯梯度值
