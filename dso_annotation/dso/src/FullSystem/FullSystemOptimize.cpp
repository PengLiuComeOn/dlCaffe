...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
	for(int k=min;k<max;k++)
	{
		...
		(*stats)[0] += r->linearize(&Hcalib); // 计算每个激活residual的雅克比矩阵，保存到本身的成员变量RawResidualJacobian *J中
		...
		if(fixLinearization)
		{
			...
			if(r->efResidual->isActive())
			{
				if(r->isNew)
				{
                    ...
                    /*个人理解：ptp_inf是深度为无穷远时的情况，深度为无穷大的点投影到图像的点为基点（消隐点），判断该点离基点的距离，如果小于10个像素（保存3d点那里使用），则不保存该点，否则从该点投影出去的位置为无穷远*/
...
{
	if(setting_solverMode & SOLVER_MOMENTUM) //（(int)128 | (int)2048） & (int)512 ：结果为0  判断setting_solverMode是不是SOLVER_MOMENTUM
	{
		if(backupLastStep)            //判断是否是第一次迭代：是第一次迭代
		{
			Hcalib.step_backup = Hcalib.step;  //备份相机内参优化的的步长
			Hcalib.value_backup = Hcalib.value;   //对相机内参进行备份
			for(FrameHessian* fh : frameHessians)    //遍历滑动窗口中的每一帧
			{
				fh->step_backup = fh->step;         //备份每帧优化状态时的步长，该状态包含10个量：[0-5: worldToCam-leftEps. 6-7: a,b]
                fh->state_backup = fh->get_state();  //对每帧的状态做备份
				for(PointHessian* ph : fh->pointHessians) //遍历滑动窗口里每一帧上的所有激活点
				{
					ph->idepth_backup = ph->idepth;  //对激活点的逆深度进行备份
					ph->step_backup = ph->step;      //备份优化逆深度时的步长
				}
			}
		}
		else                      //不是第一次迭代
		{
			Hcalib.step_backup.setZero();           //将优化相机内参的步长备份量置0
			Hcalib.value_backup = Hcalib.value;     //  对相机内参进行备份
			for(FrameHessian* fh : frameHessians)   //遍历滑动窗口里每一帧上的所有激活点
			{
				fh->step_backup.setZero();         //将优化每帧状态的步长备份量置0
				fh->state_backup = fh->get_state();   //对每帧的状态做备份
				for(PointHessian* ph : fh->pointHessians)  //遍历滑动窗口里每一帧上的所有激活点
				{
					ph->idepth_backup = ph->idepth;    //对激活点的逆深度做备份
					ph->step_backup=0;                 //将优化每个激活点的逆深度步长备份量置0
				}
			}
		}
	}
	else     //不是SOLVER_MOMENTUM
	{
		Hcalib.value_backup = Hcalib.value;      //对相机内参进行备份
		for(FrameHessian* fh : frameHessians)    //遍历滑动窗口中的每一帧
		{
			fh->state_backup = fh->get_state();   //对每帧的状态做备份，该状态包含10个量：[0-5: worldToCam-leftEps. 6-7: a,b]
			for(PointHessian* ph : fh->pointHessians)  //遍历滑动窗口里每一帧上的所有激活点
				ph->idepth_backup = ph->idepth;         //对激活点的逆深度做备份
		}
	}
...
float FullSystem::optimize(int mnumOptIts)
{
	...
	if(frameHessians.size() < 4) mnumOptIts = 15;  //根据优化窗口的帧的个数更改迭代次数
    ...
    //计算activeResiduals中每个点的jacobi，返回值为公式4的累积huber loss
	Vec3 lastEnergy = linearizeAll(false);
    
	double lastEnergyL = calcLEnergy();  //直接返回0
    
	double lastEnergyM = calcMEnergy();  //直接返回0

    
    //多线程执行：对每个残差对象的雅克比矩阵进行一些内部结合计算；
	if(multiThreading)
		treadReduce.reduce(boost::bind(&FullSystem::applyRes_Reductor, this, true, _1, _2, _3, _4), 0, activeResiduals.size(), 50);
	else
		applyRes_Reductor(true,0,activeResiduals.size(),0,0);  //将计算的J矩阵从PointFrameResidual对象中拷贝到与它对应的EFResidual对象中
	...
	{
		// solve!
		backupState(iteration!=0);   //该函数主要是备份每一个激活点的逆深度值，备份每一帧的pose和a,b， 备份相机内参；
		//solveSystemNew(0);
        
        //计算一次迭代的梯度下降方向，构造Hessian矩阵
    ...
    //设置各帧相对pose关系，含K以及delta相关数据结构
	setPrecalcValues();

    //更改了pose，需要重新线性化点，计算jacobi
...
void FullSystem::solveSystem(int iteration, double lambda)
{
	ef->lastNullspaces_forLogging = getNullspaces(
			...
			ef->lastNullspaces_affB);   //获取EnergyFuncitonal上一次pose、scale、affA、affB的Nullspaces用于日志记录

    //构造Hessian矩阵
	ef->solveSystemF(iteration, lambda,&Hcalib);  //lambda = 0.1
}
...
void FullSystem::removeOutliers()
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		for(unsigned int i=0;i<fh->pointHessians.size();i++)
		{
            ...
            //清理outlier,pointHessians中满足不含residuals的点
		...
		std::vector<VecX> &nullspaces_affB)
{
	...
	int n=CPARS+frameHessians.size()*8;  //海森矩阵优化变量包括4个相机内参，所有帧上的pose和a,b,所以优化变量数为：4+8*N， N是帧数
	std::vector<VecX> nullspaces_x0_pre;
	for(int i=0;i<6;i++)   //pose 6个变量， 循环6次，
	{
		VecX nullspace_x0(n);
	...
	for(int i=0;i<2;i++)   //光度优化变量2个a,b，循环2次；
	{
		VecX nullspace_x0(n);
		nullspace_x0.setZero();
		for(FrameHessian* fh : frameHessians)
