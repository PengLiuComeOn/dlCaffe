		...
		IOWrap::Output3DWrapper* wrap)
{
	...
	for(int lvl=coarsestLvl; lvl>=0; lvl--)
	{
        ...
        //计算误差项，保存误差相关结果和后续计算jacobi需要用到的数据
...
CoarseDistanceMap::~CoarseDistanceMap()
{
    ...
    //（作用：将激活点重投影到新帧的第1层金字塔图像上，并扩展待激活的点）
		...
		FrameHessian* frame)
{
    ...
    // fwdWarpedIDDistFinal用于代表待激活点的一个阈值，范围0-40或1000，1000代表该点未被选中，它用于后续在图像中均匀选点）
	...
	for(FrameHessian* fh : frameHessians)
	{
		...
		SE3 fhToNew = frame->PRE_worldToCam * fh->PRE_camToWorld; //当前帧到最新帧的Pose
		Mat33f KRKi = (K[1] * fhToNew.rotationMatrix().cast<float>() * Ki[0]);//将当前帧第0层金子塔坐标投影到最新帧第1层金字塔
		...
		for(PointHessian* ph : fh->pointHessians)
		{
            ...
            //判断坐标是否落在图像区域内，不满足则设置为0 （应该是满足的设置为0吧？？）
			fwdWarpedIDDistFinal[u+w1*v]=0;
			bfsList1[numItems] = Eigen::Vector2i(u,v);  //记录滑动窗口里所有激活点重投影到最新帧上的坐标
			numItems++;
		}
	}

    //不满足点领域的点做一次扩张（满足的点），向点周围扩展，将周围点也加入到bfsList1中
...
//(作用：将已有的重投影点向周围扩展，将周围点也加入到bfsList1中)
void CoarseDistanceMap::growDistBFS(int bfsNum)
{
	...
	{
		int bfsNum2 = bfsNum;
		std::swap<Eigen::Vector2i*>(bfsList1,bfsList2); //利用swap函数交换bfsList1 指针和bfsList2指针
		bfsNum=0;
