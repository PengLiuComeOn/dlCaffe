...
int main(int argc, char *argv[])
{
    ...
    float sigmaP = sigma_P_Arg.getValue();  //0.25
    ...
    while(std::getline(images_file,images_line))
    {
        ...
        if(debugFlag)
        {
            std::string imgName= "./images/" + cams_images[imgID];
        ...
        if(images_line.substr(0,1).compare("#") != 0)
        {
            ...
            else
            {
                if(cams_K.find(camID) != cams_K.end())
                {
                        ...
                        if(wpID.length() > 0)
                        {
                            ...
                            if(wp >= 0 && y > 500)
                            {
                                ...
                                if(debugFlag)
                                {
                                    ...
                                    pts.x = x;
                                    pts.y = y;
                                    cv::circle(imgPts, pts, 1, cv::Scalar(0, 255, 0), 2);
                                }
                            }
                        }
                        else
                    ...
                    if(debugFlag) // for debug
                    {
                        cv::imshow("debug", imgPts);
                        cv::waitKey(0);
                    }
...
//#endif //L3DPP_OPENMP
    for(unsigned int i=0; i<img_seq.size(); ++i)
    {
        ...
        if(cams_R.find(imgID) != cams_R.end())
        {
            ...
            std::cout << "Image: " << inputFolder+"/"+cams_images[imgID] << std::endl;
    ...
    // match images
    Line3D->matchImages(sigmaP,sigmaA,neighbors,epipolarOverlap,     //sigmaP: 2.5 sigmaA: 10.0, neighbors: 10 (the minest is 2), epipolarOverlap: 0.25 (the max is 0.99)
    kNN,constRegDepth);                                              //kNN: 10  constRegDepth: -1
    ...
    // save as STL
    Line3D->saveResultAsSTL(outputFolder, wps_coords);
    // save as OBJ
