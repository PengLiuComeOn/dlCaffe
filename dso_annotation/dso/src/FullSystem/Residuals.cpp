...
double PointFrameResidual::linearize(CalibHessian* HCalib)
{
	...
	const Mat33f &PRE_KRKiTll = precalc->PRE_KRKiTll;   //kRki
	const Vec3f &PRE_KtTll = precalc->PRE_KtTll;        //Kt
	const Mat33f &PRE_RTll_0 = precalc->PRE_RTll_0;     //R
	const Vec3f &PRE_tTll_0 = precalc->PRE_tTll_0;      //t
	...
	float d_d_x, d_d_y;
	{
		...
		if(!projectPoint(point->u, point->v, point->idepth_zero_scaled, 0, 0,HCalib,
				PRE_RTll_0,PRE_tTll_0, drescale, u, v, Ku, Kv, KliP, new_idepth))   //target u,v相机坐标系里的归一化3D点，Ku,Kv为target图像的2D点  drescale 为第一个相机坐标系下的逆深度的倒数乘以第二相机坐标系下的深度再求倒：1/(idepth1 * depth2)  KliP : host图像2D点投影到host相机坐标系下的3D点， new_idepth表示在target相机坐标系下的逆深度idepht2
			{ state_NewState = ResState::OOB; return state_energy; }

		centerProjectedTo = Vec3f(Ku, Kv, new_idepth);
        
		// diff target图像的2d坐标对d_idepth求导（这里的drescale是否应该替换为new_idepth？？？-->不应该，因为在confluence的公式推导中少了一个深度信息depth1）
		d_d_x = drescale * (PRE_tTll_0[0]-PRE_tTll_0[2]*u)*SCALE_IDEPTH*HCalib->fxl();  // 1/Z*fx(tx - tz*X/Z)
		d_d_y = drescale * (PRE_tTll_0[1]-PRE_tTll_0[2]*v)*SCALE_IDEPTH*HCalib->fyl();  //  1/Z *fy (ty - tz*Y/Z)

        // diff target图像的2d坐标对相机内参calib求导    (X' Y' 是host相机坐标系下的3D坐标, u = X/Z v = Y/Z)  （这里的drescale不应该替换为new_idepth，因为在confluence的公式推导中少了一个深度信息depth1）
		d_C_x[2] = drescale*(PRE_RTll_0(2,0)*u-PRE_RTll_0(0,0));    //  1/Z*(X/Z*r20-r00)   (r指R中的元素)
		d_C_x[3] = HCalib->fxl() * drescale*(PRE_RTll_0(2,1)*u-PRE_RTll_0(0,1)) * HCalib->fyli();  // fx/(Z*fy)*(X/Z*r21-r01)
		d_C_x[0] = KliP[0]*d_C_x[2];    //  1/Z*(X/Z*r20-r00) * X'   (d_C_x[0]与d_C_x[2]是公式推导是反的)
        d_C_x[1] = KliP[1]*d_C_x[3];    //  fx/(Z*fy)*(X/Z*r21-r01) * Y'   (d_C_x[1]与d_C_x[3]是公式推导是反的)

		d_C_y[2] = HCalib->fyl() * drescale*(PRE_RTll_0(2,0)*v-PRE_RTll_0(1,0)) * HCalib->fxli();  // fy/(Z*fx)(Y/Z*r20-r10)
		d_C_y[3] = drescale*(PRE_RTll_0(2,1)*v-PRE_RTll_0(1,1));  //  1/Z*(Y/Z*r21 - r11)
		d_C_y[0] = KliP[0]*d_C_y[2];   //    fy/(Z*fx)(Y/Z*r20-r10) * X'
		d_C_y[1] = KliP[1]*d_C_y[3];   //    1/Z*(Y/Z*r21 - r11) * Y'
        ...
        //diff pose
		d_xi_x[0] = new_idepth*HCalib->fxl();      //  fx/Z
		d_xi_x[1] = 0;                             //   0
		d_xi_x[2] = -new_idepth*u*HCalib->fxl();   // - X*fx/(Z*Z)
		d_xi_x[3] = -u*v*HCalib->fxl();            // -X*Y*fx/(Z*Z)
		d_xi_x[4] = (1+u*u)*HCalib->fxl();         // (1+X*X/(Z*Z))*fx
		d_xi_x[5] = -v*HCalib->fxl();              // -fx*Y/Z

		d_xi_y[0] = 0;                             // 0
		d_xi_y[1] = new_idepth*HCalib->fyl();      // fy/Z
		d_xi_y[2] = -new_idepth*v*HCalib->fyl();   // -Y/(Z*Z)*fx
		d_xi_y[3] = -(1+v*v)*HCalib->fyl();        // -fy*(1+Y*Y/(Z*Z))
		d_xi_y[4] = u*v*HCalib->fyl();             // fy*(X*Y/(Z*Z))
		d_xi_y[5] = u*HCalib->fyl();               // X/Z*fy
	}
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		projectedTo[idx][1] = Kv;   // 重投影到target帧的2D坐标
...

		{
			...
			J->JIdx[0][idx] = hitColor[1];   // target图像上的dx  像素值对坐标求导 dx
			J->JIdx[1][idx] = hitColor[2];   // target图像上的dy  像素值对坐标求导 dy
			J->JabF[0][idx] = drdA*hw;       // 像素对a求导 =:原图像的像素值 (gray - b)*hw
			J->JabF[1][idx] = hw;            // 像素对a求导 hw
            
            //为方便运算，提取做的一些计算
			JIdxJIdx_00+=hitColor[1]*hitColor[1];  //target图像上的dx*dx
			JIdxJIdx_11+=hitColor[2]*hitColor[2];  //target图像上的dy*dy
			JIdxJIdx_10+=hitColor[1]*hitColor[2];  //target图像上的dx*dy
