...
namespace L3DPP
{
    ...
    View::View(const unsigned int id, L3DPP::DataArray<float4>* lines,
        ...
        imgRgb_(imgRgb), superpixels_(superpixels)
    {
        // init
        ...
        Kinv_ = K_.inverse();
        Rt_ = R_.transpose(); //R.t()
        RtKinv_  = Rt_*Kinv_;  //R.t() * K.inv();
        C_ = Rt_ * (-1.0 * t_);
    ...
    //------------------------------------------------------------------------------
    void View::computeSpatialRegularizer(const float r) // r = 2.5
    {
        k_ = getSpecificSpatialReg(r);
    }

    //------------------------------------------------------------------------------
    float View::getSpecificSpatialReg(const float r) // r = 2.5
    {
        Eigen::Vector3d pp_shifted = pp_+Eigen::Vector3d(r,0.0,0.0);  // pp_ = [cx, cy , 1]; -> pp_shifted = [cx + 2.5, cy , 1]
        Eigen::Vector3d ray_pp = getNormalizedRay(pp_);  //ray_pp = normalized( R * t * Kinv * pp_);
        Eigen::Vector3d ray_pp_shifted = getNormalizedRay(pp_shifted); // ray_pp_shifted = normalized( R * t * Kinv * pp_shifted);
    ...
    {
        Eigen::Vector3d ray = RtKinv_*p;   //R.t() * k.inv() * P
    ...
    //------------------------------------------------------------------------------
    /*getNormalizedRay函数的理解，物理意义： 该函数主要用于计算（在世界坐标系下）相机光心Po与图像中间位置Puv（cx, cy， 1） == pp_投影到3D空间Pw的连线所构成的向量
     计算公式推倒： Puv = K * (R * Pw + t); (R.inv() = R.t())
     R.t() * K.inv() * Puv =  Pw + R.t() * t = Pw - (- R.t() * t) = Pw - Po = r1;
     */
    double View::opticalAxesAngle(L3DPP::View* v)
    {
        Eigen::Vector3d r1 = getNormalizedRay(pp_);  // for camId: pp_ = [cx, cy , 1];  r1 = R.t()*K.inv()*pp_ , then  normalized r1
        Eigen::Vector3d r2 = v->getOpticalAxis();    // for neighber Id :  r2 = RtKinv_*pp_  then normalized r2;
    ...
    float View::distanceVisualNeighborScore(L3DPP::View* v)
    {
        ...
        Eigen::Vector3d Ctgt_t = R_*v->C()+t_; // Ctgt_t: 将v的光心从世界坐标系转到当前相机坐标系

        // define two planes trough the camera center
        Eigen::Vector3d n1(1,0,0);  // 当前相机坐标系的x轴
        Eigen::Vector3d n2(0,1,0);  // 当前相机坐标系的y轴

        // compute distances to the planes
        float dist1 = fabs(n1.dot(Ctgt_t)); //当前相机坐标系下， v的光心到x轴的投影， Ctgt_t的x坐标值
        float dist2 = fabs(n2.dot(Ctgt_t)); //当前相机坐标系下， v的光心到y轴的投影，  Ctgt_t的y坐标值

        return dist1+dist2;     //Ctgt_t的x坐标值与y坐标值之和，
    }

    //------------------------------------------------------------------------------
    float View::baseLine(L3DPP::View* v)  // 计算相机光心的模长，基线长度
    {
        return (C_ - v->C()).norm();
    }
