...
//EFResidual:记录一个点从一帧投影到另外一帧投影残差。。
...
public:
	...
	RawResidualJacobian* J;  //RawResidualJacobian用于保存每个点投影残差的雅克比矩阵

	EIGEN_ALIGN16 VecNRf res_toZeroF;
    EIGEN_ALIGN16 Vec8f JpJdF;  // 前六项：坐标对pose求导的矩阵（6*2） * [dx*dx, dx*dy; dx*dy, dy*dy] * 像素坐标对深度求导（2*1），后两项：[a的导数*dx,a的导数*dy; b的导数*dx, b的导数*dy] * 像素坐标对深度求导
...
//EFPoint:记录一个点PointHessian所在位置EFFrame *host，到其他帧的所有投影残差，vector<EFResidual>
class EFPoint
{
public:
	...
	std::vector<EFResidual*> residualsAll;   //residualsAll: 记录一个激活点PointHessian所在位置EFFrame *host，到其他帧的所有投影残差
...
//EFFrame: 可以通过EFFrame找到帧指针，通过帧指针可以找到对应帧上的EFPoint
class EFFrame
{
public:
	...
	std::vector<EFPoint*> points;  //每帧FrameHessian *data上有很多点 vector<EFPoint *> points
	FrameHessian* data;     //与EFFrame对应的FrameHessian对象的指针
	int idx;	// idx in frames. idx表示EFFrame对象在EnergyFunctional::vector<EFFrame*> frames中的存储序号)

    int frameID;  //和FrameHessian中的frameID一样，指所有关键帧容器allKeyFramesHistory<shell>里的的序号）
