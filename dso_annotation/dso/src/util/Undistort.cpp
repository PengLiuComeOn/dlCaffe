...
//file: gamma   从pcalib.txt文件中读取G参数，用于光度校正；
		...
		int w_, int h_)
{
...

	{
        ...
        for(int i=0;i<GDepth;i++) G[i] = 255.0 * (G[i] - min) / (max-min);			//  make it to 0..255 => 0..255.不等间隔;
	}

	if(setting_photometricCalibration==0)
	{
        for(int i=0;i<GDepth;i++) G[i]=255.0f*i/(float)(GDepth-1);				//等间隔确定G
	}



	printf("Reading Vignette Image from %s\n",vignetteImage.c_str());
	MinimalImage<unsigned short>* vm16 = IOWrap::readImageBW_16U(vignetteImage.c_str());  // 读取vignette.png图像，该图像大小和要处理图像一样大；
...
Undistort* Undistort::getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename)
{
    ...
    // for backwards-compatibility: Use Pinhole / FoV model for 5 parameter.   　//执行时选用FOV模型
			...
			&ic[0], &ic[1], &ic[2], &ic[3], &ic[4]) == 5)
	{
		...
		else
		{
            ...
            u = new UndistortFOV(configFilename.c_str(), true);     //　矫正图像的内参，读取的文件不是要用的真正内参，需要矫正，最终的到K矩阵
			if(!u->isValid()) {delete u; return 0; }
		}
	}
	...
	u->loadPhotometricCalibration(
				gammaFilename,
				"",
				vignetteFilename);     //相机内参确定后，进行光标定，这里输入参数都为空，所以不进行
