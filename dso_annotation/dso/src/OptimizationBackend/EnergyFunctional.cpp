...
void EnergyFunctional::accumulateAF_MT(MatXX &H, VecX &b, bool MT)   //MT 决定是否需要加入多线程
{
	if(MT)
	{
	...
	{
        //accSSE_top_A为AccumulatedTopHessianSSE类
		accSSE_top_A->setZero(nFrames);  //为每一帧（EFFrame）构建AccumulatorApprox对象
		for(EFFrame* f : frames)
			for(EFPoint* p : f->points)
...
void EnergyFunctional::accumulateSCF_MT(MatXX &H, VecX &b, bool MT)
{
	...
	{
		accSSE_bot->setZero(nFrames);  //初始化AccumulatedSCHessianSSE类对象的成员变量，赋为0值
		for(EFFrame* f : frames)
			for(EFPoint* p : f->points)
...
EFFrame* EnergyFunctional::insertFrame(FrameHessian* fh, CalibHessian* Hcalib)
{
	...
	setAdjointsF(Hcalib);
	makeIDX(); //   //更新frames对象和EFResidual对象的序号
...
void EnergyFunctional::solveSystemF(int iteration, double lambda, CalibHessian* HCalib)
{
    ...
    //由以下三个函数来构建Hessien Matrix
	accumulateAF_MT(HA_top, bA_top,multiThreading);  //AF=active

	accumulateLF_MT(HL_top, bL_top,multiThreading);  //LF=Linearize

	accumulateSCF_MT(H_sc, b_sc,multiThreading);    //sc=marginlize
